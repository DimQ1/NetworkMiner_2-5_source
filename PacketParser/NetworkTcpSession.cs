...
{
    public class NetworkTcpSession : IComparable<NetworkTcpSession>
    {//only TCP sessions
        ...
        public NetworkTcpSession(Packets.TcpPacket tcpSynPacket, NetworkHost clientHost, NetworkHost serverHost, ISessionProtocolFinderFactory protocolFinderFactory, Func<DateTime, string> toCustomTimeZoneStringFunction)
        {
            ...
            {//It's normal to start the session with a SYN flag
        ...
        public NetworkTcpSession(NetworkHost sourceHost, NetworkHost destinationHost, Packets.TcpPacket tcpPacket, ISessionProtocolFinderFactory protocolFinderFactory, Func<DateTime, string> toCustomTimeZoneStringFunction)
        {
            ...
            if (sourcePortProtocols.Count > destinationPortProtocols.Count)
            ...
            else if (destinationPortProtocols.Count > 0)
        ...
        public bool TryAddPacket(Packets.TcpPacket tcpPacket, NetworkHost sourceHost, NetworkHost destinationHost)
        {
            ...
            else if (sourceHost == this.ServerHost && tcpPacket.SourcePort == this.ServerTcpPort)
            {//server -> client
                ...
                try
                {
                    ...
                    if (tcpPacket.FlagBits.Acknowledgement && tcpPacket.FlagBits.Fin && tcpPacket.FlagBits.Push)
                    {
                        ...
                        if (sourceHost.IPAddress.Equals(this.ServerHost.IPAddress) && tcpPacket.SourcePort == this.ServerTcpPort)
                            ...
                            this.clientToServerTcpDataStream.ExpectedTcpSequenceNumber = tcpPacket.SequenceNumber;
                    }
        ...
        public class TcpDataStream
        {
            ...
            {
                get { return this.initialTcpSequenceNumber; }
                set { this.initialTcpSequenceNumber = value; }
            }
            internal uint ExpectedTcpSequenceNumber
            {
                get { return this.expectedTcpSequenceNumber; }
                set { this.expectedTcpSequenceNumber = value; }
            }
            ...
            public void AddTcpData(uint tcpSequenceNumber, byte[] tcpSegmentData)
            {
                ...
                {//It is VERY important that no 0 length data arrays are added! There is otherwise a big risk for getting stuck in forever-loops etc.
                    ...
                    if ((int)(expectedTcpSequenceNumber - tcpSequenceNumber) <= 0 && tcpSequenceNumber - expectedTcpSequenceNumber < 1234567)
                    {
                        ...
                        {//let's replace the old TCP packet with the new one
