    ...
    public class SimpleParser : ISimpleParser {
        ...
        public void Parse(string pcapFileName) {
            using (PcapFileReader pcapReader = new PcapFileReader(pcapFileName)) {
                ...
                PacketParser.PacketHandler packetHandler = new PacketHandler(executablePath, Environment.CurrentDirectory, null, true, new Func<DateTime, string>((DateTime dateTime) => { return dateTime.ToUniversalTime().ToString("u"); }), false);
