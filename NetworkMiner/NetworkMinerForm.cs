    ...
    public partial class NetworkMinerForm : Form
    {
        ...
        public GuiProperties GuiProperties
        {
            ...
            set
            {
                ...
                this.setTabPageVisibility(this.tabPageVoIP, this.guiProperties.UseVoipTab);
        ...
        public NetworkMinerForm(bool showWinPcapAdapterMissingError, bool removeBrowsersAndVoipTab)
        {
            ...
            */
            ...
            try
            {//not all versions of mono support codepage 932 for some reason
            ...
            try
            {//not all versions of mono support codepage 936 for some reason
            ...
            try
            {//not all versions of mono support codepage 949 for some reason
            ...
            try
            {//not all versions of mono support codepage 1251 for some reason
            ...
            if (removeBrowsersAndVoipTab)
            {
                ...
                this.guiProperties.UseVoipTab = false;
            }
        ...
        private void AddFilesToFileList(IEnumerable<PacketParser.FileTransfer.ReconstructedFile> files)
        {
            ...
            foreach (PacketParser.FileTransfer.ReconstructedFile file in files)
            {
                ...
                {
                    if (isAdvertisment)
                        item.ForeColor = this.guiProperties.AdvertisementColor;
                    else if (isTracker)
        ...
        private void AddDnsRecordsToDnsList(IEnumerable<PacketParser.Events.DnsRecordEventArgs> records)
        {
            ...
            foreach (PacketParser.Events.DnsRecordEventArgs r in records)
            {
                ...
                ListViewItem item = new ListViewItem(
                    ...
                    this.guiProperties.ToCustomTimeZoneString(r.Record.ParentPacket.ParentFrame.Timestamp),//.ToString(),
        ...
        {
            if (wordCharCount * 16 > totalByteCount)
            {//at least 1/16 of the data shall be clear text, otherwise it is probable to be false positives
                StringBuilder sb = new StringBuilder();
        ...
        private void resizeListViewColumns(ListView lv)
        {
            ...
            if (PacketParser.Utils.SystemHelper.IsRunningOnMono()) {
                ...
                for (int i = 0; i < lv.Columns.Count; i++) {
                    if (!logged) {
                        SharedUtils.Logger.Log("Width = " + lv.Columns[i].Width.ToString(), System.Diagnostics.EventLogEntryType.Information);
                        logged = true;
                    }
                    if (lv.Columns[i].Width < 33) {
                        lv.Columns[i].Width = 33;
                        
                    }
                    else if (lv.Columns[i].Width > this.GuiProperties.ColumnAutoResizeMaxWidth)
            ...
            */
            if (this.GuiProperties.ColumnAutoResizeMaxWidth > 0)
            {
                ...
                for (int i = 0; i < lv.Columns.Count; i++)
                {
                    ...
                    else if (PacketParser.Utils.SystemHelper.IsRunningOnMono() && lv.Columns[i].Width < 33)
                    {
                        lv.Columns[i].Width = 33;
                    }
...
2. Start NetworkMiner with administrator rights.");
        ...
        private void UpdateGuiControlsAfterParsingCompleteEventHandler(object sender, EventArgs e)
        {
            ...
            if (this.guiProperties.UseVoipTab && this.VoipCallHandler != null)
                foreach (VoipCall voipCall in this.VoipCallHandler.GetVoipCalls(this.guiProperties.ToCustomTimeZoneString))
        ...
        void fileWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            using (PcapFileReader pcapReader = lp.PcapReader)
            {
                ...
                */
                ...
                while (packetHandlerWrapper.PacketHandler.FramesInQueue > 0)
                {//just to make sure we dont finish too early
            ...
            try
            {
                ...
                //SharedUtils.Logger.Log("PCAP loading window closed...", System.Diagnostics.EventLogEntryType.Information);
            }
            catch (Exception ex)
            {
        ...
        private bool TryAddKeywordToListBox(string keyword, out string errorMessage)
        {
            ...
            else
            {
                ...
                if (this.keywordTextBox.Text.StartsWith("0x"))
                {//hex string
                    try
                    {
        ...
        class ListViewItemComparer : System.Collections.IComparer, IComparer<ListViewItem>
        {
            private int col;
            ...
            if (e.Item is ListViewItem listViewItem)
            {
                ...
                if (listViewItem?.Tag != null)
                {
                    ...
                    if (System.IO.File.Exists(filePath))
                    {
                        ...
                        {//Pokemon exception mode: catch 'em all
    ...
    $pipeStream.Dispose();
        ...
        private void setAdvertismentColorToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (this.colorDialog1.ShowDialog() == DialogResult.OK)
                {
                    this.guiProperties.AdvertisementColor = this.colorDialog1.Color;
        ...
        private void filesListView_SelectedIndexChanged(object sender, EventArgs e)
        {
            lock (filesListView.ContextMenuStrip)
            {
                ...
                if (filesMenuStripExtraItems.Count > 0)
                    filesMenuStripExtraItems.Insert(0, new ToolStripSeparator());
                filesListView.ContextMenuStrip.Items.AddRange(filesMenuStripExtraItems.ToArray());
            }
        }
